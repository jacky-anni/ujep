...
const EditStudents: React.FC<{}> = () => {
  ...
  if (currentStudent.student) {
    initialValues = {
      nom: currentStudent.student.person.nom,
      prenom: currentStudent.student.person.prenom,
      sexe: currentStudent.student.person.sexe,
      email: currentStudent.student.person.email,
      telephone: currentStudent.student.person.telephone,
      date_naissance: currentStudent.student.person.date_naissance,
      lieu_naissance: currentStudent.student.lieu_naissance,
      groupe_sanguin: currentStudent.student.groupe_sanguin,
      statut_matrimonial: currentStudent.student.statut_matrimonial,
      cin: "32323232",
      nif: currentStudent.student.nif,
    };
  } else {
    initialValues = {
      nom: "",
  ...
  const form = (
    <Formik
      ...
      onSubmit={async (values, { resetForm }) => {
        ...
        setLoading(true);
        // insert employee
        const data = await editStudents(
          currentStudent.student.person.uuid,
          values
        );
        // if errors
        if (data.status === 404) {
          ...
          return navigate("/dashbord/profile-students/" + data.person.uuid);
        }
      }}
    >
      {({ errors, touched }) => (
        ...
        <div className='col-lg-12'>
          <div className='card'>
            <div className='card-body'>
              ...
              <AppTitle title={"Modifier cet etudiant"} />
              {form}
            </div>{" "}
            {/* end card-body*/}
          </div>{" "}
